...
└── src/
    ...
    └── public/
        ...
        └── js/
            ...
            ├── asistenteCore.js   ← factory `crearAsistenteChat(elementos)` con toda la lógica del asistente (conexión Gemini Live, audio, tool-calling, historial); instanciado tanto por la página completa como por el widget. Incluye llamada de voz abierta con VAD (botón "📞 Llamada"/"🛑 Cortar"), nota de voz estilo WhatsApp (botón 🎤: mantener presionado graba, soltar envía, deslizar arriba fija manos-libres, deslizar a la izquierda cancela) y envío de texto — captura de audio PCM16 unificada en el helper interno `crearCapturaAudio()`, compartido entre llamada y nota
...
- **El texto tipeado al asistente se manda con `sendClientContent`, no con `sendRealtimeInput` (bug real, PR #5):** `sendRealtimeInput` del SDK `@google/genai` solo tiene un campo `media` (Blob de audio/video) — no existe ningún campo `text` en su interfaz, así que `session.sendRealtimeInput({ text: texto })` descartaba el texto en silencio, sin tirar error, y por eso el asistente nunca "escuchaba" lo tipeado. Verificado contra la documentación del SDK vía context7 antes de tocar el código. El fix cambia a `session.sendClientContent({ turns: [{ role: "user", parts: [{ text: texto }] }] })`, que es el método correcto del SDK para mandar contenido que no es un Blob de audio/video en vivo.
- **Nota de voz estilo WhatsApp usa `audioStreamEnd: true`, no espera el VAD del servidor:** al soltar el botón 🎤 (o tocar ✔️ en el overlay fijado), se manda `session.sendRealtimeInput({ audioStreamEnd: true })` para cortar el turno al instante, en vez de depender del detector de silencio (VAD) que ya usa la llamada de voz abierta — la nota tiene un final explícito (soltar el dedo), así que no tiene sentido esperar a que el servidor detecte silencio. Señal confirmada contra la documentación del SDK vía context7.
- **Captura de audio PCM16 unificada en `crearCapturaAudio()`:** antes de esta sesión, la llamada de voz (`activarMic`/`detenerMic`) y la nota de voz (nueva) hubieran duplicado el mismo pipeline `getUserMedia` + `AudioContext` + `ScriptProcessor` + conversión a base64. Se extrajo a un helper compartido dentro de `asistenteCore.js` que además resuelve, al centralizar el estado, un bug de reentrancia (un tap muy rápido en el botón de nota antes de terminar de conectar podía arrancar dos grabaciones superpuestas y dejar el micrófono/AudioContext colgado) — encontrado en el `/code-review high` previo a mergear.
...
- **Validación de horario de turnos en punto o y media (PR #3, commit `472ef43`, 2026-08-06):** al crear o editar un turno, el horario ahora se exige en punto o a la media hora (`:00` o `:30`) — ver detalle en "Pendiente / sin definir" más abajo, este cambio no se llegó a documentar en `TASKS.md`/`PROGRESS.md` en su momento.
- **Ocultar "Horas trabajadas" en Mi Panel y sincronizar % ganancia al reasignar turno (PR #4, commit `7960305`, 2026-09-06):** en `/mi-panel` (vista de empleada) se sacó la tarjeta "Horas trabajadas" de las 5 secciones de métricas (semana actual, anterior, siguiente, mes, filtro) — la métrica sigue calculándose igual en `calcularMetricas` (`dateHelpers.js`), solo se dejó de mostrar. En `agenda/editar.ejs`, al reasignar un turno a otra empleada, el campo "% Ganancia de la empleada en este turno" ahora se autocompleta con el `porcentaje_ganancia` configurado para la empleada recién seleccionada (antes quedaba el valor de la empleada anterior si no se tocaba a mano, y el backend guarda ese campo tal cual viene del form sin recalcularlo — era un bug real de integridad de datos, no solo cosmético).
- **Fix de texto del asistente + llamada/nota de voz rediseñadas (PR #5, rama `feat/asistente-input-texto`, commits `945b691` y `6151fb9`, 2026-09-11 — pendiente de mergear a `main`):** el texto tipeado en el chat del asistente no llegaba nunca al modelo (`sendRealtimeInput` no tiene campo `text`, se descartaba en silencio); se corrigió usando `sendClientContent` — ver detalle en "Decisiones de diseño relevantes". De paso se rediseñó la interacción de audio: el botón de llamada abierta pasó a llamarse "📞 Llamada"/"🛑 Cortar" (mismo comportamiento VAD de siempre, sin cambios funcionales) y se sumó un botón 🎤 de nota de voz estilo WhatsApp (mantener presionado graba, soltar envía, deslizar arriba fija manos-libres con overlay ✔️/✕, deslizar a la izquierda cancela), usando `audioStreamEnd: true` para cortar el turno al instante en vez de esperar el VAD del servidor. Todo vive una sola vez en `crearAsistenteChat()`, compartido entre `/asistente` y el widget flotante. Un `/code-review high` previo a mergear encontró y corrigió 3 bugs reales (reset indebido del lock al tocar el botón de nota ya fijada, `pointercancel` tratado como envío en vez de descarte, falta de guard de reentrada en `iniciarGrabacionNota`) y una duplicación de código (pipeline de captura de audio extraído al helper `crearCapturaAudio()`).
...
- PR #5 (`feat/asistente-input-texto`) todavía no está mergeado a `main` — https://github.com/DepaoloJuan/Sol---Admin/pull/5. No se pudo probar en un browser real el gesto de mantener presionado/deslizar de la nota de voz (necesita permiso real de micrófono); se verificó por lectura de código y `/code-review high`, falta que un humano lo pruebe con el dedo o mouse antes de confiar 100% en la UX. No hay tests automatizados ni para el fix de texto ni para la nota de voz. Queda un edge case menor sin corregir a propósito (no justifica el costo ahora): si se suelta el botón de nota mientras `iniciarGrabacionNota()` todavía espera `conectar()`/permiso de mic, y en esa ventana se hace un swipe-cancelar en vez de soltar simple, el swipe no se detecta (solo el release simple vía `notaPendienteDetener`) y queda pendiente de "enviar" en vez de "cancelar" — ventana de tiempo muy chica, no bloqueante.
- `TASKS.md`/`PROGRESS.md` no llegaron a documentar en su momento el PR #3 (`fix/validar-horario-turnos-30min`, commit `472ef43`, 2026-08-06) — ya está en producción, exige que el horario de un turno (creación/edición) sea en punto o y media, pero el detalle de implementación no quedó registrado en la bitácora ni en el roadmap. Si hace falta ese nivel de detalle a futuro, conviene reconstruirlo desde el diff del commit antes de que se pierda contexto.
